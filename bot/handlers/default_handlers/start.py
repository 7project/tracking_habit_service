...
def bot_start(message: Message):
    bot.reply_to(message, f"{message.from_user.full_name}!\n"
                          ...
                          f"Время оповещения фиксировано: 08:15 10:15 12:15 15:15 18:15 20:15 21:45 МСК\n"
                          f"Если в первый раз запустил бота, тогда нужно авторизоваться. Введи учетные данные ниже, придумай логин и пароль.\n"
                          f"Если ранее аккаунт был создан и есть, нажми /cancel для выхода из состояния команды, и введи команду\n"
                          f"/token для обновления токена, нужно будет ввести пароль от ранее созданного пользователя.\n"
                          f"В приложении пользователь привязан к ID TELEGRAM: {message.from_user.id}\n"
                          f"Используя сервис, вы разрешаете использовать ваши персональные данные \nдля обработки запросов и хранение ваших привычек.")
